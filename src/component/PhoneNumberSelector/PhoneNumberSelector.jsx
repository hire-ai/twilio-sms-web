...
const PhoneNumberSelector = ({
  ...
  const handleGetPhoneNumberSuccess = useCallback(
    (response) => {
      ...
      const retrievedNumbers = response
        ...
        .map((pn) => ({ number: pn?.phone_number, name: pn?.friendly_name }))
        .sort((a, b) => a.name.localeCompare(b.name));
      console.log("retrievedNumbers", retrievedNumbers);
  ...
  const phoneNumberOptions = phoneNumbers.map((v) => ({
    value: v.number,
    label: `${v.name}: (${maskPhoneNumber(v.number)})`,
  }));
